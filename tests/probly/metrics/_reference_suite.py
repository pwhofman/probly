...
class ReferenceSuite:
    ...
    @_ROUNDS
    def test_auc_matches_sklearn_with_ties(self, _round, array_fn):  # noqa: PT019
        """Auc over roc_curve matches sklearn when scores contain tied blocks."""
        rng = np.random.default_rng()
        y_true = rng.integers(0, 2, size=30).astype(float)
        y_score = rng.integers(0, 4, size=30) / 4.0  # few distinct values guarantee ties
    ...
    @_ROUNDS
    def test_roc_auc_score_matches_sklearn_with_ties(self, _round, array_fn):  # noqa: PT019
        """roc_auc_score matches sklearn when scores contain tied blocks."""
        rng = np.random.default_rng()
        y_true = rng.integers(0, 2, size=30).astype(float)
        y_score = rng.integers(0, 4, size=30) / 4.0  # few distinct values guarantee ties
    ...
    @_ROUNDS
    def test_average_precision_score_matches_sklearn_with_ties(self, _round, array_fn):  # noqa: PT019
        """average_precision_score matches sklearn when scores contain tied blocks."""
        rng = np.random.default_rng()
        y_true = rng.integers(0, 2, size=30).astype(float)
        y_score = rng.integers(0, 4, size=30) / 4.0  # few distinct values guarantee ties
