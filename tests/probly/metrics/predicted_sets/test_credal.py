...
def _categorical(probs: np.ndarray) -> NumpyCategoricalDistribution:
...
@pytest.fixture
def make_convex():
    return lambda probs: NumpyConvexCredalSet(array=_categorical(np.asarray(probs)))


@pytest.fixture
def make_distance():
    return lambda nominal, radius: NumpyDistanceBasedCredalSet(
...
@pytest.fixture
def make_intervals():
    return lambda lower, upper: NumpyProbabilityIntervalsCredalSet(
...
class TestSingletonCredalSet:
    def test_coverage_collapses_to_top1(self) -> None:
        ...
        cs = NumpySingletonCredalSet(array=_categorical(probs))
    ...
    def test_efficiency_is_one(self) -> None:
        cs = NumpySingletonCredalSet(array=_categorical(np.array([[0.5, 0.5]])))
...
class TestDiscreteCredalSet:
    def test_coverage_uses_any_vertex_argmax(self) -> None:
        ...
        cs = NumpyDiscreteCredalSet(array=_categorical(probs))
...
class TestConvexCredalSet:
    def test_integer_label_coverage(self) -> None:
        ...
        cs = NumpyConvexCredalSet(array=_categorical(probs))
...
class TestRoundingInProbVectorCoverage:
    ...
    def test_near_zero_lower_does_not_block_zero_target(self) -> None:
        ...
        cs = NumpyProbabilityIntervalsCredalSet(lower_bounds=lower, upper_bounds=upper)
...
class TestDistanceBasedCredalSet:
    def test_coverage_and_efficiency(self) -> None:
        ...
        radius = np.array([0.1])
        cs = NumpyDistanceBasedCredalSet(nominal=nominal, radius=radius)
...
def test_empty_input_returns_nan() -> None:
    ...
    cs = NumpyProbabilityIntervalsCredalSet(
