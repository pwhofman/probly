"""Tests for the JaxSample Representation."""
...
def assert_weights_equal(sample: JaxSample, expected: object) -> None:
    assert sample.weights is not None
    assert np.array_equal(np.asarray(sample.weights), np.asarray(expected))


class TestJaxSample:
    def test_sample_internal_array(self, jax_array_sample_2d: JaxSample) -> None:
        assert isinstance(jax_array_sample_2d.array, jnp.ndarray)

    def test_sample_length(self, jax_array_sample_2d: JaxSample) -> None:
        assert len(jax_array_sample_2d) == len(jax_array_sample_2d.array)

    def test_sample_ndim(self, jax_array_sample_2d: JaxSample) -> None:
        assert jax_array_sample_2d.ndim == 2

    def test_sample_shape(self, jax_array_sample_2d: JaxSample) -> None:
        assert jax_array_sample_2d.shape == jax_array_sample_2d.array.shape

    def test_sample_move_axis(self, jax_array_sample_2d: JaxSample) -> None:
        moved_sample = jax_array_sample_2d.move_sample_axis(0)
        assert isinstance(moved_sample, JaxSample)
    ...
    def test_sample_concat(self, jax_array_sample_2d: JaxSample) -> None:
        res = jax_array_sample_2d.concat(jax_array_sample_2d.move_sample_axis(0))
        assert isinstance(res, JaxSample)
    ...
    def test_from_iterable_preserves_weights(self) -> None:
        ...
        sample = JaxSample.from_iterable(jnp.arange(6).reshape((3, 2)), sample_axis=0, weights=weights)

        assert_weights_equal(sample, weights)

    def test_constructor_rejects_wrong_weight_shape(self) -> None:
        with pytest.raises(ValueError, match="weights must have shape"):
            JaxSample(jnp.arange(12).reshape((3, 4)), sample_axis=1, weights=jnp.array([0.1, 0.2, 0.3]))

    def test_from_sample_preserves_weights(self) -> None:
        weights = jnp.array([0.1, 0.2, 0.3, 0.4])
        sample = JaxSample(jnp.arange(12).reshape((3, 4)), sample_axis=1, weights=weights)

        converted = JaxSample.from_sample(sample, sample_axis=0)
    ...
    def test_sample_concat_combines_weights(self) -> None:
        left = JaxSample(jnp.arange(12).reshape((3, 4)), sample_axis=1, weights=jnp.array([0.1, 0.2, 0.3, 0.4]))
        right = JaxSample(jnp.arange(12, 24).reshape((4, 3)), sample_axis=0, weights=jnp.array([0.5, 0.6, 0.7, 0.8]))
    ...
    def test_sample_concat_fills_missing_weights_with_ones(self) -> None:
        left = JaxSample(jnp.arange(12).reshape((3, 4)), sample_axis=1)
        right = JaxSample(jnp.arange(12, 24).reshape((3, 4)), sample_axis=1, weights=jnp.array([0.5, 0.6, 0.7, 0.8]))
    ...
    def test_weighted_sample_var_rejects_ddof(self) -> None:
        sample = JaxSample(jnp.arange(12).reshape((3, 4)), sample_axis=1, weights=jnp.ones(4))
...
class TestJaxSampleEdgeCases:
    """JaxSample validation and operations."""
    ...
    def test_weights_shape_mismatch(self) -> None:
        ...
        with pytest.raises(ValueError, match="weights must have shape"):
            JaxSample(jnp.zeros((2, 3)), sample_axis=0, weights=jnp.zeros(5))

    def test_T_property(self) -> None:  # noqa: N802
        _, jnp = _jax_modules()
        from probly.representation.sample.jax import JaxSample  # noqa: PLC0415

        s = JaxSample(jnp.arange(6).reshape(2, 3), sample_axis=0)
    ...
    def test_dtype_property(self) -> None:
        ...
        s = JaxSample(jnp.arange(6, dtype=jnp.float32).reshape(2, 3), sample_axis=0)
    ...
    def test_ndim_property(self) -> None:
        ...
        s = JaxSample(jnp.zeros((2, 3, 4)), sample_axis=0)
        assert s.ndim == 3
    ...
    def test_samples_property_moves_axis(self) -> None:
        ...
        s = JaxSample(jnp.arange(24).reshape(2, 3, 4), sample_axis=2)
    ...
    def test_samples_property_axis_zero(self) -> None:
        ...
        s = JaxSample(jnp.arange(24).reshape(2, 3, 4), sample_axis=0)
    ...
    def test_sample_size(self) -> None:
        ...
        s = JaxSample(jnp.zeros((5, 3)), sample_axis=0)
    ...
    def test_sample_std_unweighted(self) -> None:
        ...
        s = JaxSample(jnp.array([1.0, 2.0, 3.0]), sample_axis=0)
    ...
    def test_concat_two_jax_samples(self) -> None:
        ...
        a = JaxSample(jnp.arange(6).reshape(2, 3), sample_axis=0)
        b = JaxSample(jnp.arange(6, 12).reshape(2, 3), sample_axis=0)
        c = a.concat(b)
    ...
    def test_move_sample_axis(self) -> None:
        ...
        a = JaxSample(jnp.arange(24).reshape(2, 3, 4), sample_axis=0)
    ...
    def test_copy(self) -> None:
        ...
        a = JaxSample(jnp.arange(6).reshape(2, 3), sample_axis=0, weights=jnp.array([0.5, 0.5]))
        c = a.copy()
    ...
    def test_from_iterable_auto_axis(self) -> None:
        ...
        s = JaxSample.from_iterable(jnp.arange(12).reshape(3, 4))
        # auto -> -1
        assert s.sample_axis == 1
    ...
    def test_from_iterable_zero_dim_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="Cannot infer"):
            JaxSample.from_iterable(jnp.array(5))
    ...
    def test_from_iterable_with_dtype(self) -> None:
        ...
        s = JaxSample.from_iterable(jnp.arange(12).reshape(3, 4), dtype=jnp.float32)
    ...
    def test_from_iterable_explicit_axis(self) -> None:
        ...
        s = JaxSample.from_iterable(jnp.arange(12).reshape(3, 4), sample_axis=0)
        assert s.sample_axis == 0


@pytest.fixture
def sample_3d() -> JaxSample:
    """A 3-D sample whose sample axis sits in the middle."""
    return JaxSample(jnp.arange(24, dtype=jnp.float32).reshape(2, 3, 4), sample_axis=1)


@pytest.fixture
def weighted_sample_2d() -> JaxSample:
...
class TestJaxSampleIsJaxLike:
    """The migration to ``JaxLikeImplementation`` and the members it now provides."""

    def test_is_jax_like_implementation(self, sample_3d: JaxSample) -> None:
        assert isinstance(sample_3d, JaxLikeImplementation)

    def test_is_usable_as_a_plain_jax_array(self, sample_3d: JaxSample) -> None:
        # The numpy backend's NumpySample works with plain numpy through ``__array__``; the jax
    ...
    def test_from_iterable_of_a_traced_array_does_not_unroll(self) -> None:
        ...
        def build(array: jax.Array) -> jax.Array:
            sample = JaxSample.from_iterable(array, sample_axis=0)
    ...
    def test_transpose_property_tracks_sample_axis(self, sample_3d: JaxSample) -> None:
        result = sample_3d.T
    ...
    def test_matrix_transpose_property_tracks_sample_axis(self, sample_3d: JaxSample) -> None:
        result = sample_3d.mT
    ...
    def test_adjoint_property_tracks_sample_axis(self, sample_3d: JaxSample) -> None:
        result = sample_3d.mH
    ...
    def test_transpose_method_with_axes(self, sample_3d: JaxSample) -> None:
        result = sample_3d.transpose(2, 1, 0)

        assert isinstance(result, JaxSample)
        assert result.sample_axis == 1

    def test_at_exposes_indexed_update_helper(self, sample_3d: JaxSample) -> None:
        updated = sample_3d.at[0, 0, 0].set(99.0)

        assert isinstance(updated, jax.Array)
        assert float(updated[0, 0, 0]) == pytest.approx(99.0)

    def test_block_until_ready_returns_self(self, weighted_sample_2d: JaxSample) -> None:
        assert weighted_sample_2d.block_until_ready() is weighted_sample_2d

    def test_astype_casts_the_array(self, sample_3d: JaxSample) -> None:
    ...
    def test_getitem_tracks_the_sample_axis(self, sample_3d: JaxSample) -> None:
        result = sample_3d[0]
    ...
    def test_getitem_indexes_weights_on_the_sample_axis(self, weighted_sample_2d: JaxSample) -> None:
        result = weighted_sample_2d[:, jnp.array([3, 1])]

        assert isinstance(result, JaxSample)
        assert_weights_equal(result, jnp.array([0.4, 0.2]))

    def test_iteration_yields_samples(self, sample_3d: JaxSample) -> None:
        items = list(sample_3d)

        assert len(items) == 2
        assert all(isinstance(item, JaxSample) for item in items)

    def test_setitem_is_rejected(self, sample_3d: JaxSample) -> None:
...
class TestJaxFunctionReductions:
    ...
    def test_reduction_over_other_axis_shifts_sample_axis(self, sample_3d: JaxSample, func: object) -> None:
    ...
    @pytest.mark.parametrize("func", [jax_mean, jax_sum, jax_std, jax_var, jax_average])
    def test_reduction_over_sample_axis_drops_the_wrapper(self, sample_3d: JaxSample, func: object) -> None:
        result = func(sample_3d, 1)  # ty: ignore[call-non-callable]

        assert not isinstance(result, JaxSample)
        assert result.shape == (2, 4)

    def test_reduction_over_multiple_axes(self, sample_3d: JaxSample) -> None:
    ...
    def test_reduction_over_negative_axis(self, sample_3d: JaxSample) -> None:
        result = jax_mean(sample_3d, -1)

        assert isinstance(result, JaxSample)
        assert result.sample_axis == 1

    def test_keepdims_keeps_the_sample_axis(self, sample_3d: JaxSample) -> None:
    ...
    def test_reduction_over_all_axes_drops_the_wrapper(self, sample_3d: JaxSample) -> None:
        result = jax_sum(sample_3d)

        assert not isinstance(result, JaxSample)

    def test_average_ignores_the_sample_weights(self, weighted_sample_2d: JaxSample) -> None:
    ...
    def test_average_over_all_axes_ignores_the_sample_weights(self, weighted_sample_2d: JaxSample) -> None:
    ...
    def test_sample_mean_uses_the_sample_weights(self, weighted_sample_2d: JaxSample) -> None:
    ...
    def test_average_keeps_explicit_weights(self, weighted_sample_2d: JaxSample) -> None:
    ...
    def test_reduction_preserves_weights_off_the_sample_axis(self, weighted_sample_2d: JaxSample) -> None:
    ...
    def test_std_and_var_honour_ddof(self, sample_3d: JaxSample) -> None:
...
class TestJaxFunctionAxisMoves:
    ...
    def test_transpose_without_axes(self, sample_3d: JaxSample) -> None:
    ...
    def test_transpose_with_axes(self, sample_3d: JaxSample) -> None:
    ...
    def test_transpose_with_negative_axes(self, sample_3d: JaxSample) -> None:
        result = jax_transpose(sample_3d, (-2, -1, -3))

        assert isinstance(result, JaxSample)
        assert result.sample_axis == 0

    def test_matrix_transpose(self, sample_3d: JaxSample) -> None:
    ...
    def test_matrix_transpose_leaves_other_axes_alone(self) -> None:
        sample = JaxSample(jnp.zeros((2, 3, 4)), sample_axis=0)
    ...
    def test_moveaxis_moves_the_sample_axis(self, sample_3d: JaxSample) -> None:
    ...
    def test_moveaxis_of_another_axis_shifts_the_sample_axis(self, sample_3d: JaxSample) -> None:
    ...
    def test_moveaxis_with_negative_axes(self, sample_3d: JaxSample) -> None:
        result = jax_moveaxis(sample_3d, -2, 0)

        assert isinstance(result, JaxSample)
        assert result.sample_axis == 0

    def test_moveaxis_with_sequences(self, sample_3d: JaxSample) -> None:
    ...
    def test_conj_preserves_the_sample_axis(self, sample_3d: JaxSample) -> None:
...
class TestJaxFunctionShapeChanges:
    ...
    def test_copy_preserves_the_sample_axis(self, weighted_sample_2d: JaxSample) -> None:
    ...
    def test_reshape_keeps_a_surviving_sample_axis(self, sample_3d: JaxSample) -> None:
    ...
    def test_reshape_drops_a_merged_sample_axis(self, sample_3d: JaxSample) -> None:
        result = jax_reshape(sample_3d, (2, 12))

        assert not isinstance(result, JaxSample)
        assert result.shape == (2, 12)

    def test_reshape_matches_the_numpy_backend(self, sample_3d: JaxSample) -> None:
        array_sample = NumpySample(np.asarray(sample_3d.array), sample_axis=sample_3d.sample_axis)

        result = jax_reshape(sample_3d, (2, 3, 4, 1))
        expected = np.reshape(array_sample, (2, 3, 4, 1))

        assert isinstance(result, JaxSample)
        assert isinstance(expected, NumpySample)
        assert result.sample_axis == expected.sample_axis

    def test_flatten_returns_a_bare_array(self, sample_3d: JaxSample) -> None:
        assert not isinstance(sample_3d.flatten(), JaxSample)
        assert not isinstance(sample_3d.ravel(), JaxSample)

    def test_squeeze_shifts_the_sample_axis(self) -> None:
        sample = JaxSample(jnp.zeros((1, 2, 3)), sample_axis=1)
    ...
    def test_squeeze_of_the_sample_axis_drops_the_wrapper(self) -> None:
        sample = JaxSample(jnp.zeros((2, 1, 3)), sample_axis=1)
    ...
    def test_expand_dims_shifts_the_sample_axis(self, sample_3d: JaxSample) -> None:
    ...
    def test_expand_dims_after_the_sample_axis_keeps_it(self, sample_3d: JaxSample) -> None:
        result = jax_expand_dims(sample_3d, -1)

        assert isinstance(result, JaxSample)
        assert result.sample_axis == 1

    def test_swapaxes_follows_the_sample_axis(self, sample_3d: JaxSample) -> None:
    ...
    def test_swapaxes_of_other_axes_keeps_the_sample_axis(self) -> None:
        sample = JaxSample(jnp.zeros((2, 3, 4)), sample_axis=1)
    ...
    def test_take_along_axis_off_the_sample_axis_keeps_it(self, sample_3d: JaxSample) -> None:
    ...
    def test_take_along_axis_on_the_sample_axis_drops_the_wrapper(self, sample_3d: JaxSample) -> None:
...
class TestJaxFunctionSequences:
    ...
    def test_concatenate_along_the_sample_axis(self, sample_3d: JaxSample) -> None:
    ...
    def test_concatenate_along_another_axis(self, sample_3d: JaxSample) -> None:
    ...
    def test_concatenate_with_a_plain_array(self, sample_3d: JaxSample) -> None:
        result = jax_concatenate([sample_3d, sample_3d.array], 1)

        assert isinstance(result, JaxSample)
        assert result.shape == (2, 6, 4)

    def test_concatenate_with_mixed_sample_axes_returns_a_plain_array(self, sample_3d: JaxSample) -> None:
        other = JaxSample(sample_3d.array, sample_axis=0)

        result = jax_concatenate([sample_3d, other], 0)

        assert not isinstance(result, JaxSample)

    def test_concatenate_combines_weights(self, weighted_sample_2d: JaxSample) -> None:
    ...
    def test_concatenate_fills_missing_weights_with_ones(self, weighted_sample_2d: JaxSample) -> None:
        unweighted = JaxSample(weighted_sample_2d.array, sample_axis=1)
    ...
    def test_concatenate_of_weighted_samples_off_the_sample_axis_raises(self, weighted_sample_2d: JaxSample) -> None:
    ...
    def test_stack_before_the_sample_axis(self, sample_3d: JaxSample) -> None:
    ...
    def test_stack_after_the_sample_axis(self, sample_3d: JaxSample) -> None:
    ...
    def test_stack_with_negative_axis(self, sample_3d: JaxSample) -> None:
    ...
    def test_stack_of_weighted_samples_raises(self, weighted_sample_2d: JaxSample) -> None:
        with pytest.raises(ValueError, match="do not support stack"):
            jax_stack([weighted_sample_2d, weighted_sample_2d], 0)


class TestJaxSampleConversions:
    ...
    def test_conversion_preserves_nested_distribution(self, distribution_type, conversion: str) -> None:
        ...
        sample = JaxSample(distribution_type(values), sample_axis=1, weights=weights)
        ...
        assert isinstance(converted, JaxSample)
    ...
    def test_round_trip_back_to_array_sample(self) -> None:
        ...
        assert isinstance(round_tripped, NumpySample)
    ...
    def test_jax_like_without_arguments_returns_self(self, sample_3d: JaxSample) -> None:
        assert sample_3d.__jax_like__() is sample_3d

    def test_jax_like_with_dtype_casts(self, sample_3d: JaxSample) -> None:
...
class TestJaxSamplePytree:
    """``JaxSample`` participates in JAX transformations as a pytree."""

    def test_tree_map_round_trip(self, sample_3d: JaxSample) -> None:
        mapped = jax.tree.map(lambda leaf: leaf * 2, sample_3d)

        assert isinstance(mapped, JaxSample)
        assert mapped.sample_axis == sample_3d.sample_axis
        np.testing.assert_allclose(np.asarray(mapped), np.asarray(sample_3d) * 2)

    def test_tree_map_keeps_weights_as_children(self, weighted_sample_2d: JaxSample) -> None:
        leaves = jax.tree.leaves(weighted_sample_2d)

        assert len(leaves) == 2

    def test_tree_flatten_puts_the_sample_axis_in_aux_data(self, sample_3d: JaxSample) -> None:
        children, aux = sample_3d.tree_flatten()

        assert children[0] is sample_3d.array
        assert ("sample_axis", 1) in aux[1]

    def test_jit_round_trip(self, sample_3d: JaxSample) -> None:
        @jax.jit
        def double(sample: JaxSample) -> JaxSample:
            return JaxSample(sample.array * 2, sample.sample_axis)
    ...
    def test_vmap_over_a_sample(self, sample_3d: JaxSample) -> None:
