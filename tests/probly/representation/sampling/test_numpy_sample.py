"""Tests for the NumpySample Representation."""
...
from probly.representation.sample.numpy_functions import numpy_sample_internals


def assert_weights_equal(sample: NumpySample, expected: object) -> None:
...
class TestNumpySample:
    ...
    def test_sample_length(self, array_sample_2d: NumpySample[int]) -> None:
        assert len(array_sample_2d) == len(array_sample_2d.array)

    def test_sample_ndim(self, array_sample_2d: NumpySample[int]) -> None:
        assert array_sample_2d.ndim == 2

    def test_sample_shape(self, array_sample_2d: NumpySample[int]) -> None:
        assert array_sample_2d.shape == array_sample_2d.array.shape

    def test_sample_move_axis(self, array_sample_2d: NumpySample[int]) -> None:
        moved_sample = array_sample_2d.move_sample_axis(0)
        assert isinstance(moved_sample, NumpySample)
    ...
    def test_sample_concat(self, array_sample_2d: NumpySample[int]) -> None:
        res = array_sample_2d.concat(array_sample_2d.move_sample_axis(0))
        assert isinstance(res, NumpySample)
    ...
    def test_from_iterable_preserves_weights(self) -> None:
        ...
        sample = NumpySample.from_iterable(np.arange(6).reshape((3, 2)), sample_axis=0, weights=weights)

        assert_weights_equal(sample, weights)

    def test_constructor_rejects_wrong_weight_shape(self) -> None:
        with pytest.raises(ValueError, match="weights must have shape"):
            NumpySample(np.arange(12).reshape((3, 4)), sample_axis=1, weights=np.array([0.1, 0.2, 0.3]))

    def test_from_sample_preserves_weights(self) -> None:
        weights = np.array([0.1, 0.2, 0.3, 0.4])
        sample = NumpySample(np.arange(12).reshape((3, 4)), sample_axis=1, weights=weights)

        converted = NumpySample.from_sample(sample, sample_axis=0)
    ...
    def test_sample_concat_combines_weights(self) -> None:
        left = NumpySample(np.arange(12).reshape((3, 4)), sample_axis=1, weights=np.array([0.1, 0.2, 0.3, 0.4]))
        right = NumpySample(np.arange(12, 24).reshape((4, 3)), sample_axis=0, weights=np.array([0.5, 0.6, 0.7, 0.8]))
    ...
    def test_sample_concat_fills_missing_weights_with_ones(self) -> None:
        left = NumpySample(np.arange(12).reshape((3, 4)), sample_axis=1)
        right = NumpySample(np.arange(12, 24).reshape((3, 4)), sample_axis=1, weights=np.array([0.5, 0.6, 0.7, 0.8]))
    ...
    def test_sample_mean_of_categorical_distribution_preserves_distribution_type(self) -> None:
        ...
        sample = NumpySample(NumpyProbabilityCategoricalDistribution(probabilities), sample_axis=0)

        result = sample.sample_mean()

        assert isinstance(result, NumpyCategoricalDistribution)
    ...
    def test_weighted_sample_mean_of_categorical_distribution_uses_weights(self) -> None:
        ...
        sample = NumpySample(NumpyProbabilityCategoricalDistribution(probabilities), sample_axis=0, weights=weights)
    ...
    def test_weighted_sample_var_rejects_ddof(self) -> None:
        sample = NumpySample(np.arange(12).reshape((3, 4)), sample_axis=1, weights=np.ones(4))

        with pytest.raises(ValueError, match="ddof"):
            sample.sample_var(ddof=1)

    def test_sample_slicing(self, array_sample_2d: NumpySample[int]) -> None:
        indexed_sample = array_sample_2d[:, :3]

        assert isinstance(indexed_sample, NumpySample)
        assert indexed_sample.sample_axis == 1
        assert indexed_sample.shape == (3, 3)

    def test_sample_selection(self, array_sample_2d: NumpySample[int]) -> None:
        indexed_sample = array_sample_2d[:, 3]
    ...
    def test_weighted_integer_on_sample_axis_returns_array(self) -> None:
        sample = NumpySample(np.arange(12).reshape((3, 4)), sample_axis=1, weights=np.array([0.1, 0.2, 0.3, 0.4]))
    ...
    def test_weighted_multidimensional_boolean_index_touching_sample_axis_raises(self) -> None:
        sample = NumpySample(np.arange(12).reshape((3, 4)), sample_axis=0, weights=np.array([0.1, 0.2, 0.3]))
    ...
    def test_unweighted_complex_indexing_still_works(self) -> None:
        sample = NumpySample(np.arange(12).reshape((3, 4)), sample_axis=1)

        indexed_sample = sample[:, np.array([[0, 1]])]

        assert isinstance(indexed_sample, np.ndarray)

    def test_ufunc_call(self, array_sample_2d: NumpySample[int]) -> None:
        result = array_sample_2d + 5

        assert isinstance(result, NumpySample)
    ...
    def test_ufunc_reduce_along_non_sample(self, array_sample_2d: NumpySample[int]) -> None:
    ...
    def test_ufunc_reduce_along_sample(self, array_sample_2d: NumpySample[int]) -> None:
    ...
    def test_array_function_concatenate_preserves_sample_axis(self, array_sample_2d: NumpySample[int]) -> None:
        other = NumpySample(np.arange(12, 24).reshape((3, 4)), sample_axis=1)
    ...
    def test_array_function_squeeze_non_sample_axis_preserves_weights(self) -> None:
        weights = np.array([0.1, 0.2, 0.3, 0.4])
        sample = NumpySample(np.arange(4).reshape((1, 4)), sample_axis=1, weights=weights)
    ...
    def test_array_function_squeeze_sample_axis_drops_type(self) -> None:
        sample = NumpySample(np.arange(3).reshape((3, 1)), sample_axis=1, weights=np.array([0.1]))
    ...
    def test_array_function_concatenate_drops_type_on_sample_axis_mismatch(
        self, array_sample_2d: NumpySample[int]
    ) -> None:
        other = NumpySample(np.arange(12, 24).reshape((3, 4)), sample_axis=0)

        result = np.concatenate((array_sample_2d, other), axis=0)

        assert isinstance(result, np.ndarray)

    def test_array_function_concatenate_axis_none_drops_type(self, array_sample_2d: NumpySample[int]) -> None:
        result = np.concatenate((array_sample_2d, array_sample_2d), axis=None)

        assert isinstance(result, np.ndarray)

    def test_array_function_concatenate_with_sample_out_returns_out(self, array_sample_2d: NumpySample[int]) -> None:
        out = NumpySample(np.empty((3, 8), dtype=array_sample_2d.dtype), sample_axis=0)
    ...
    def test_array_function_stack_with_weights_raises(self) -> None:
        weights = np.array([0.1, 0.2, 0.3, 0.4])
        left = NumpySample(np.arange(12).reshape((3, 4)), sample_axis=1, weights=weights)
        right = NumpySample(np.arange(12, 24).reshape((3, 4)), sample_axis=1, weights=weights)

        with pytest.raises(ValueError, match="stack"):
            np.stack((left, right), axis=0)

    def test_array_function_stack_keeps_sample_axis_when_axis_is_after(self, array_sample_2d: NumpySample[int]) -> None:
    ...
    def test_array_function_stack_drops_type_on_sample_axis_mismatch(self, array_sample_2d: NumpySample[int]) -> None:
        other = NumpySample(np.arange(12, 24).reshape((3, 4)), sample_axis=0)
    ...
    def test_array_function_stack_with_sample_out_returns_out(self, array_sample_2d: NumpySample[int]) -> None:
        out = NumpySample(np.empty((2, 3, 4), dtype=array_sample_2d.dtype), sample_axis=0)
...
class TestNumpySampleValidation:
    ...
    def test_array_must_be_arraylike(self) -> None:
        ...
        with pytest.raises((TypeError, AttributeError)):
            NumpySample(array=[[1, 2], [3, 4]], sample_axis=0)  # type: ignore[arg-type]

    def test_weights_shape_mismatch(self) -> None:
        arr = np.zeros((2, 3))
        with pytest.raises(ValueError, match="weights must have shape"):
            NumpySample(array=arr, sample_axis=0, weights=np.zeros(5))


class TestNumpySampleArrayFunctions:
    ...
    def test_copy_with_subok_true_preserves_sample_axis(self) -> None:
        ...
        assert isinstance(copied, NumpySample)
    ...
    def test_copy_with_subok_false_returns_plain_array(self) -> None:
        ...
        assert not isinstance(copied, NumpySample)
    ...
    def test_mean_along_sample_axis_drops_sample_wrapping(self) -> None:
        ...
        assert not isinstance(result, NumpySample)
    ...
    def test_matrix_transpose_swaps_last_two_axes(self) -> None:
        arr = np.arange(24).reshape(2, 3, 4)
        sample = NumpySample(array=arr, sample_axis=2)
...
class TestNumpySampleConcatenateStack:
    """Concatenate/stack with weights and out= variants."""

    def test_concat_axis_negative_normalised(self) -> None:
        a = NumpySample(array=np.arange(6).reshape(2, 3), sample_axis=0)
        b = NumpySample(array=np.arange(6, 12).reshape(2, 3), sample_axis=0)
...
class TestNumpySampleSpecialMethods:
    """Conversion, dunder and detach-style methods."""

    def test_array_namespace(self) -> None:
        sample = NumpySample(array=np.arange(6).reshape(2, 3), sample_axis=0)
        assert sample.__array_namespace__() is not None

    def test_dtype_property(self) -> None:
        sample = NumpySample(array=np.arange(6).reshape(2, 3).astype(np.float64), sample_axis=0)
    ...
    def test_iter_yields_along_first_axis(self) -> None:
        sample = NumpySample(array=np.array([[1, 2], [3, 4]]), sample_axis=0)
    ...
    def test_bool_one_element(self) -> None:
        ...
        sample = NumpySample(array=np.array([True]), sample_axis=0)
        assert bool(sample) is True

    def test_bool_zero_element(self) -> None:
        sample = NumpySample(array=np.array([False]), sample_axis=0)
    ...
    def test_index_dunder(self) -> None:
        ...
        # We can't easily build a scalar NumpySample (rejected by __post_init__)
        # so we just check that the method exists and forwards.
        sample = NumpySample(array=np.array([5]), sample_axis=0)
    ...
    def test_setitem(self) -> None:
        sample = NumpySample(array=np.arange(6).reshape(2, 3).astype(float), sample_axis=0)
        sample[0, 0] = 99
    ...
    def test_copy_method(self) -> None:
        sample = NumpySample(array=np.arange(6).reshape(2, 3), sample_axis=1)
        c = sample.copy()
        assert isinstance(c, NumpySample)
    ...
    def test_eq_returns_array(self) -> None:
        a = NumpySample(array=np.array([1, 2, 3]), sample_axis=0)
        b = a == 2
    ...
    def test_hash_works(self) -> None:
        ...
        b = NumpySample(array=np.array([1, 2, 3]), sample_axis=0)
        assert hash(a) != hash(b)


class TestNumpySampleStats:
    """Aggregate statistics methods on NumpySample."""

    def test_sample_mean_with_weights(self) -> None:
        a = NumpySample(array=np.array([1.0, 2.0, 3.0]), sample_axis=0, weights=np.array([1.0, 0.0, 0.0]))
...
class TestNumpySampleIndexing:
    """Indexing semantics for NumpySample including weighted samples."""
...
class TestUFuncReductions:
    """NumpySample interactions with numpy ufunc reductions / outer / at."""
    ...
    def test_ufunc_outer_drops_wrapper(self) -> None:
        sample = NumpySample(array=np.array([1.0, 2.0]), sample_axis=0)
        result = np.add.outer(sample, np.array([10.0, 20.0]))
        assert not isinstance(result, NumpySample)

    def test_ufunc_at_drops_wrapper(self) -> None:
        sample = NumpySample(array=np.arange(4).astype(float), sample_axis=0)
        np.add.at(sample, [0, 1], 1.0)
        np.testing.assert_array_equal(sample.array, [1.0, 2.0, 2.0, 3.0])


class TestNumpySampleInternalsRegistry:
    """The single-dispatch registry returns expected internals for NumpySample."""

    def test_returns_internals_for_array_sample(self) -> None:
        sample = NumpySample(array=np.arange(6).reshape(2, 3), sample_axis=0)
        internals = numpy_sample_internals(sample)
    ...
    def test_returns_none_for_unknown_input(self) -> None:
        assert numpy_sample_internals(object()) is None


class TestNumpySampleFromSample:
    """Ensure NumpySample.from_sample handles various input types."""

    def test_from_sample_with_existing_array_sample(self) -> None:
        original = NumpySample(array=np.arange(6).reshape(2, 3), sample_axis=0)
        result = NumpySample.from_sample(original)
        np.testing.assert_array_equal(result.array, original.array)
        assert result.sample_axis == 0

    def test_from_sample_with_dtype(self) -> None:
        original = NumpySample(array=np.arange(6, dtype=np.int32).reshape(2, 3), sample_axis=0)
        result = NumpySample.from_sample(original, dtype=np.float64)
        assert result.array.dtype == np.float64

    def test_from_sample_moves_axis(self) -> None:
        original = NumpySample(array=np.arange(6).reshape(2, 3), sample_axis=0)
        result = NumpySample.from_sample(original, sample_axis=1)
        assert result.sample_axis == 1
        # Shape rotates accordingly.
        assert result.array.shape == (3, 2)


class TestNumpySampleFromIterable:
    """from_iterable edge cases."""

    def test_with_empty_iterable_raises(self) -> None:
        with pytest.raises(ValueError, match="Cannot infer"):
            NumpySample.from_iterable([], sample_axis="auto")

    def test_with_zero_dim_array_raises(self) -> None:
        # Passing a 0-dim ndarray with sample_axis="auto" should fail.
        with pytest.raises(ValueError, match="Cannot infer"):
            NumpySample.from_iterable(np.array(5), sample_axis="auto")

    def test_with_explicit_sample_axis(self) -> None:
        s = NumpySample.from_iterable(np.arange(6).reshape(3, 2), sample_axis=0)
        assert s.sample_axis == 0

    def test_with_dtype(self) -> None:
        s = NumpySample.from_iterable([np.array([1.0]), np.array([2.0])], dtype=np.float32)
        assert s.array.dtype == np.float32


class TestNumpySampleConcat:
    """concat edge cases."""
    ...
    def test_concat_with_non_arraysample(self) -> None:
        ...
        a = NumpySample(array=np.array([[1.0, 2.0], [3.0, 4.0]]), sample_axis=0)
...
class TestNumpySampleUfuncOut:
    """The __array_ufunc__ ``out=`` keyword path."""

    def test_ufunc_with_explicit_out_arraysample(self) -> None:
        a = NumpySample(array=np.zeros(3, dtype=float), sample_axis=0)
        b = NumpySample(array=np.array([1.0, 2.0, 3.0]), sample_axis=0)
        out = NumpySample(array=np.zeros(3, dtype=float), sample_axis=0)
