"""Edge cases for NumpySample / TorchSample dunder methods.
...
class TestNumpySampleScalarConversionsAndDunders:
    """Cover the small dunder branches in NumpySample."""
    ...
    def test_array_dunder_with_explicit_dtype(self) -> None:
        ...
        sample = NumpySample(array=np.arange(6, dtype=np.int64).reshape(2, 3), sample_axis=0)
    ...
    def test_dtype_property(self) -> None:
        ...
        sample = NumpySample(array=np.arange(6, dtype=np.float64).reshape(2, 3), sample_axis=0)
        assert sample.dtype == np.float64

    def test_ndim_property(self) -> None:
        from probly.representation.sample.numpy import NumpySample  # noqa: PLC0415

        sample = NumpySample(array=np.zeros((2, 3, 4)), sample_axis=0)
    ...
    def test_flags_property(self) -> None:
        ...
        sample = NumpySample(array=np.zeros((2, 3)), sample_axis=0)
    ...
    def test_setitem_mutates_array(self) -> None:
        ...
        sample = NumpySample(array=np.zeros((2, 3), dtype=float), sample_axis=0)
        sample[0, 0] = 99.0
...
class TestNumpySampleToTorch:
    """The __torch_like__ conversion path in NumpySample."""
    ...
    def test_to_torch_like_basic(self) -> None:
        ...
        sample = NumpySample(array=np.arange(6, dtype=np.float32).reshape(2, 3), sample_axis=0)
...
class TestNumpySampleFromSamplePaths:
    """from_sample's lesser-used branches."""

    def test_from_sample_with_explicit_axis_moves_axis(self) -> None:
        from probly.representation.sample.numpy import NumpySample  # noqa: PLC0415

        original = NumpySample(array=np.arange(12).reshape(3, 4), sample_axis=0)
        moved = NumpySample.from_sample(original, sample_axis=1)
...
class TestNumpySampleArrayFunctionsEdges:
    ...
    def test_reduction_with_non_bool_keepdims(self) -> None:
        ...
        sample = NumpySample(array=np.arange(12, dtype=float).reshape(3, 4), sample_axis=0)
    ...
    def test_reduction_with_out_parameter(self) -> None:
        """When `out=NumpySample(...)`, the function returns the out wrapper."""
        from probly.representation.sample.numpy import NumpySample  # noqa: PLC0415

        sample = NumpySample(array=np.arange(12, dtype=float).reshape(3, 4), sample_axis=0)
        out_buf = NumpySample(array=np.zeros(3), sample_axis=0)
