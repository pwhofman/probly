"""Extra coverage for ``probly.representation.sample.numpy``.
...
class TestNumpySamplePostInitGuards:
    ...
    def test_non_array_input_raises_type_error(self) -> None:
        ...
        with pytest.raises(TypeError, match="must be a NumpyArrayLike"):
            NumpySample(array=_FakeArray(), sample_axis=0)  # type: ignore[arg-type]


class TestNumpySampleFromSampleConversionBranches:
    """``from_sample`` branches that don't go through NumpySample directly."""

    def test_from_sample_with_listsample_uses_from_iterable(self) -> None:
        """Non-NumpySample, non-Convertible samples take the ``from_iterable`` branch (line 131)."""
        from probly.representation.sample._common import ListSample  # noqa: PLC0415

        ls = ListSample([np.array([1.0, 2.0]), np.array([3.0, 4.0])])

        result = NumpySample.from_sample(ls)
...
class TestNumpySampleDeviceProperty:
...
class TestNumpySampleGetitemUntrackedIndex:
    ...
    def test_zero_d_index_drops_wrapper(self) -> None:
        # A scalar selection from a 1-D NumpySample yields a numpy scalar,
        # which has no ``.ndim`` attribute, exercising the early return.
        sample = NumpySample(array=np.array([1, 2, 3]), sample_axis=0)
        result = sample[0]
        # The returned value should not be an NumpySample (no ``ndim``).
        assert not isinstance(result, NumpySample)


class TestNumpySampleUfuncOutSingleArg:
...
class TestNumpySampleScalarConversionDunders:
    """``__int__`` / ``__float__`` / ``__complex__`` delegate to the array (lines 391, 399, 403).

    A 0-D NumpySample is rejected by ``__post_init__``, so a 1-element array is the
    ...
    def test_int_dunder_dispatches(self) -> None:
        ...
        sample = NumpySample(array=np.array([42]), sample_axis=0)
    ...
    def test_float_dunder_dispatches(self) -> None:
        ...
        sample = NumpySample(array=np.array([2.5]), sample_axis=0)
        with contextlib.suppress(TypeError, ValueError):
            sample.__float__()
    ...
    def test_complex_dunder_dispatches(self) -> None:
        ...
        sample = NumpySample(array=np.array([1.0 + 2.0j]), sample_axis=0)
        with contextlib.suppress(TypeError, ValueError):
            sample.__complex__()
