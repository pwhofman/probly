...
def test_from_array_basic() -> None:
    ...
    dist = NumpyDirichletDistribution.from_array(alphas_list, dtype=np.float32)
...
def test_len_behaviour() -> None:
    ...
    dist_single = NumpyDirichletDistribution.from_array([1.0, 2.0, 3.0])
    with pytest.raises(TypeError, match="len\\(\\) of unsized representation"):
        _ = len(dist_single)

    dist_batch = NumpyDirichletDistribution.from_array(np.ones((5, 3)))
...
def test_transpose_property() -> None:
    ...
    assert isinstance(transposed, NumpyDirichletDistribution)
...
def test_operator_add_returns_distribution_and_keeps_positive() -> None:
    ...
    dist = NumpyDirichletDistribution.from_array([[1.0, 2.0, 3.0]])

    out = dist + 1.0
    assert isinstance(out, NumpyDirichletDistribution)
    np.testing.assert_array_equal(out.alphas, dist.alphas + 1.0)

    out2 = dist - 1000.0
    assert isinstance(out2, NumpyDirichletDistribution)
...
def test_copy_creates_independent_array() -> None:
    ...
    assert isinstance(copied, NumpyDirichletDistribution)
...
def test_entropy_matches_scipy_single() -> None:
    ...
    dist = NumpyDirichletDistribution(alpha)
...
def test_sample_function_dirichlet() -> None:
    ...
    shape = (2, 3)
    dist = NumpyDirichletDistribution(np.ones(shape))
...
def test_sample_simplex_constraints() -> None:
    ...
    dist = NumpyDirichletDistribution(np.array([1.0, 2.0, 3.0]))
...
def test_sample_statistics_dirichlet_var() -> None:
    ...
    dist = NumpyDirichletDistribution(alphas)
...
def test_expand_dims_last_inserts_before_class_axis() -> None:
    ...
    assert isinstance(expanded, NumpyDirichletDistribution)
...
def test_reshape_with_none_inserts_before_class_axis() -> None:
    ...
    assert isinstance(reshaped, NumpyDirichletDistribution)
...
def test_mean_preserves_distribution_type_and_class_axis() -> None:
    ...
    assert isinstance(meaned, NumpyDirichletDistribution)
...
def test_sum_preserves_distribution_type_and_class_axis() -> None:
    ...
    assert isinstance(summed, NumpyDirichletDistribution)
...
def test_average_preserves_distribution_type_and_uses_weights() -> None:
    ...
    assert isinstance(averaged, NumpyDirichletDistribution)
