...
def test_accepts_relative_non_negative_probabilities() -> None:
    ...
    dist = NumpyProbabilityCategoricalDistribution(probabilities)
...
def test_rejects_negative_relative_probabilities() -> None:
    ...
    with pytest.raises(ValueError, match="non-negative"):
        NumpyProbabilityCategoricalDistribution(probabilities)


def test_zero_sum_relative_probabilities_return_nan() -> None:
    dist = NumpyProbabilityCategoricalDistribution(np.array([0.0, 0.0, 0.0], dtype=float))
...
def test_expand_dims_last_inserts_before_class_axis() -> None:
    ...
    assert isinstance(expanded, NumpyCategoricalDistribution)
...
def test_reshape_with_none_inserts_before_class_axis() -> None:
    ...
    assert isinstance(reshaped, NumpyCategoricalDistribution)
...
def test_concatenate_preserves_distribution_type() -> None:
    ...
    assert isinstance(concatenated, NumpyCategoricalDistribution)
...
def test_stack_preserves_distribution_type() -> None:
    ...
    assert isinstance(stacked, NumpyCategoricalDistribution)
...
def test_mean_preserves_distribution_type_and_class_axis() -> None:
    ...
    dist = NumpyProbabilityCategoricalDistribution(unnormalized)

    meaned = np.mean(dist, axis=0)

    assert isinstance(meaned, NumpyCategoricalDistribution)
...
def test_average_preserves_distribution_type_and_uses_weights() -> None:
    ...
    assert isinstance(averaged, NumpyCategoricalDistribution)
...
def test_hash_is_identity_based_and_distinguishes_instances() -> None:
    ...
    dist_a = NumpyProbabilityCategoricalDistribution(probabilities.copy())
    dist_b = NumpyProbabilityCategoricalDistribution(probabilities.copy())
...
class TestNumpyCategoricalDistributionPostprocessing:
    ...
    def test_mean_returns_probability_distribution(self) -> None:
        ...
        d = NumpyLogitCategoricalDistribution(array=np.array([[1.0, 2.0, 3.0], [3.0, 2.0, 1.0]]))
        result = np.mean(d, axis=0)
        # The mean of a logit-cat distribution should produce a probability distribution.
        assert isinstance(result, NumpyProbabilityCategoricalDistribution)
...
class TestNumpyCategoricalDistribution:
    ...
    def test_negative_probabilities_raise(self) -> None:
        ...
        with pytest.raises(ValueError, match="non-negative"):
            NumpyProbabilityCategoricalDistribution(array=np.array([0.5, -0.1, 0.6]))
    ...
    def test_zero_dim_array_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="at least one dimension"):
            NumpyProbabilityCategoricalDistribution(array=np.array(0.5))
    ...
    def test_array_must_be_ndarray(self) -> None:
        ...
        with pytest.raises(TypeError, match="numpy ndarray"):
            NumpyProbabilityCategoricalDistribution(array=[0.5, 0.5])  # type: ignore[arg-type]
    ...
    def test_logit_array_must_be_ndarray(self) -> None:
        ...
        with pytest.raises(TypeError, match="numpy ndarray"):
            NumpyLogitCategoricalDistribution(array=[0.5, 0.5])  # type: ignore[arg-type]
    ...
    def test_logit_zero_dim_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match="at least one dimension"):
            NumpyLogitCategoricalDistribution(array=np.array(0.5))
    ...
    def test_eq_two_probability_distributions(self) -> None:
        ...
        d1 = NumpyProbabilityCategoricalDistribution(array=np.array([[0.2, 0.3, 0.5]]))
        d2 = NumpyProbabilityCategoricalDistribution(array=np.array([[0.4, 0.6, 1.0]]))
    ...
    def test_logit_eq(self) -> None:
        ...
        a = NumpyLogitCategoricalDistribution(array=np.array([[1.0, 2.0, 3.0]]))
        b = NumpyLogitCategoricalDistribution(array=np.array([[1.0, 2.0, 3.0]]))
        assert bool(a == b)
    ...
    def test_hash(self) -> None:
        ...
        a = NumpyProbabilityCategoricalDistribution(array=np.array([[0.5, 0.5]]))
        b = NumpyLogitCategoricalDistribution(array=np.array([[0.0, 1.0]]))
