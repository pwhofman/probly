...
from probly.representation.distribution.numpy_categorical import NumpyLogitCategoricalDistribution
...
def test_representation_loss(rule, sample_axis: int) -> None:
    ...
    distribution = NumpyLogitCategoricalDistribution(np.log(probabilities) + 3.0)
    ...
    for values in (probabilities, distribution):
        sample = NumpySample(values, sample_axis=sample_axis, weights=weights)
