...
from probly.visualization.input_handling import (
    check_num_classes,
    check_shape,
    dispatch_plot,
    normalize_input,
...
def test_dispatch_plot_raises_label_mismatch() -> None:
    """Test Error when label count doesn't match class count."""
    data = np.array([[0.5, 0.5]])
    with pytest.raises(ValueError, match="Number of labels"):
        dispatch_plot(data, labels=["A", "B", "C"])


def test_dispatch_plot_raises_invalid_choice() -> None:
    """Test Error when choice is invalid string."""
    data = np.array([[0.5, 0.5]])
    with pytest.raises(ValueError, match="Choice must be"):
        dispatch_plot(data, choice="Invalid")
