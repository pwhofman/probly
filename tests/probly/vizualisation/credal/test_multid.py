...
def _spider(
    ...
    *,
    ...
    # NOTE: title/mle_flag/credal_flag are required in your implementation
    return viz.spider_plot(probs, labels=labels, title=title, mle_flag=mle_flag, credal_flag=credal_flag, ax=ax)
...
def test_spider_plot_returns_radar_axes_and_sets_ylim_and_title() -> None:
    ...
    # Depending on implementation, title might be custom title or "Spider Plot ..."
...
def test_spider_plot_mle_scatter_is_at_argmax_of_mean_probs() -> None:
    """Matches the plot code: MLE uses argmax of mean(axis=0)."""
    viz = MultiVisualizer()
    probs = np.array(
        [
            [0.1, 0.1, 0.1, 0.7],
...
def test_spider_plot_uses_passed_in_axes() -> None:
    ...
    # First call registers the radar projection inside the plotting module (important!)
...
def test_create_credal_plot_length_mismatch_raises() -> None:
    """Don't assume an exact error message: it may come from Matplotlib."""
    labels = ["A", "B", "C"]
