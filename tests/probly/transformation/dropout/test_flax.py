...
def test_linear_network_with_first_linear(flax_model_small_2d_2d: nnx.Sequential) -> None:
    ...
    Parameters:
        flax_model_small_2d_2d: The torch model to be tested, specified as a sequential model.
    ...
    count_linear_original = sum(1 for m in flax_model_small_2d_2d.layers if isinstance(m, nnx.Linear))
    # count number of nnx.Dropout layers in original model
    count_dropout_original = sum(1 for m in flax_model_small_2d_2d.layers if isinstance(m, nnx.Dropout))
    # count number of nnx.Sequential layers in original model
    count_sequential_original = sum(1 for m in flax_model_small_2d_2d.layers if isinstance(m, nnx.Sequential))

    # count number of nnx.Dropout layers in modified model
    count_dropout_modified = sum(1 for m in model.layers if isinstance(m, nnx.Dropout))
    # count number of nnx.Linear layers in modified model
    count_linear_modified = sum(1 for m in model.layers if isinstance(m, nnx.Linear))
    # count number of nnx.Sequential layers in modified model
    count_sequential_modified = sum(1 for m in model.layers if isinstance(m, nnx.Sequential))
...
def test_linear_network_p_value(flax_model_small_2d_2d: nnx.Sequential) -> None:
...
def test_convolutional_network(flax_conv_linear_model: nnx.Sequential) -> None:
    ...
    # count number of nnx.Linear layers in original model
    count_linear_original = sum(1 for m in flax_conv_linear_model.layers if isinstance(m, nnx.Linear))
    # count number of nnx.Dropout layers in original model
    count_dropout_original = sum(1 for m in flax_conv_linear_model.layers if isinstance(m, nnx.Dropout))
    # count number of nnx.Sequential layers in original model
    count_sequential_original = sum(1 for m in flax_conv_linear_model.layers if isinstance(m, nnx.Sequential))
    # count number of nnx.Conv2d layers in original model
    count_conv_original = sum(1 for m in flax_conv_linear_model.layers if isinstance(m, nnx.Conv))
    ...
    count_conv_modified = sum(1 for m in model.layers if isinstance(m, nnx.Conv))
...
def test_conv_network_p_value(flax_conv_linear_model: nnx.Sequential) -> None:
