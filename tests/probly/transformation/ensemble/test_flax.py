...
class TestFlaxEnsemble:
    ...
    def test_generate_ensemble_structure(self, flax_model_small_2d_2d: nnx.Sequential) -> None:
        ...
        """
        n_members = 3
        models = ensemble(flax_model_small_2d_2d, n_members)
        ...
        for layer_original, layer_ensemble in zip(flax_model_small_2d_2d.layers, models[0].layers, strict=False):
            assert type(layer_original) is type(layer_ensemble)

    def test_ensemble_independence(self, flax_model_small_2d_2d: nnx.Sequential) -> None:
        ...
        """
        n_members = 3
        ensemble_models = ensemble(flax_model_small_2d_2d, n_members)
        ...
        for updated_param, other_param in zip(updated_first, params_before[1], strict=False):
