...
def test_generate_distinct_ensembles():
...
def test_ensemble_different_sizes(n_members):
    base_model = SimpleFlaxModel()
    model = generate_flax_ensemble(base_model, n_members)
    assert model.n_members == n_members

def test_call_signature_accepts_return_all_and_kwargs():
...
def test_return_all_and_average_shape_and_values():
...
def test_pytree_output_stacking():
...
def test_return_all_and_average_shape_and_values_runtime():
...
def test_pytree_output_stacking_runtime():
