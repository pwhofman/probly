...
def test_invalid_input_in_evidential_classification(dummy_predictor: Predictor) -> None:
    """Tests the evidential classification function when provided with a model that has no registered appender."""
    with pytest.raises(
            NotImplementedError,
            match=f"No evidential classification appender registered for type {type(dummy_predictor)}"):
