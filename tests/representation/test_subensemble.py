...
def test_forward_shape(subensemble):
    x = torch.randn(2, 3, 5, 5)
    output = subensemble(x)
    assert output.shape == (2, 2)


def test_predict_pointwise_shape(subensemble):
    x = torch.randn(2, 3, 5, 5)
    output = subensemble.predict_pointwise(x)
    assert output.shape == (2, 2)


def test_predict_pointwise_logits_shape(subensemble):
    x = torch.randn(2, 3, 5, 5)
    output = subensemble.predict_pointwise(x, logits=True)
    assert output.shape == (2, 2)


def test_predict_pointwise_probabilities(subensemble):
    x = torch.randn(2, 3, 5, 5)
    output = subensemble.predict_pointwise(x)
    assert torch.allclose(output.sum(dim=1), torch.ones_like(output[:, 0]))


def test_predict_representation_shape(subensemble):
    x = torch.randn(2, 3, 5, 5)
    output = subensemble.predict_representation(x)
    assert output.shape == (2, 5, 2)


def test_predict_representation_logits_shape(subensemble):
    x = torch.randn(2, 3, 5, 5)
    output = subensemble.predict_representation(x, logits=True)
    assert output.shape == (2, 5, 2)


def test_predict_representation_probabilities(subensemble):
    x = torch.randn(2, 3, 5, 5)
    output = subensemble.predict_representation(x)
    assert torch.allclose(output.sum(dim=2), torch.ones_like(output[:, :, 0]))


def test_models_independent(subensemble):
...
def test_base_frozen_and_head_not(subensemble):
