 ...
 "cells": [
  ...
  },
  {
   ...
   "source": [
    ...
    "- Crucially, these Dropout layers can be configured to stay **active during inference**, so each forward pass is a bit different.\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "Below: build a small MLP, apply `dropout(model, p, enable_at_eval=True)`, and inspect the modified architecture.\n"
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    ...
    "\n",
    "model_do = dropout(model, p, enable_at_eval=True)\n",
    "print(\"\\nWith Dropout transformation (p=%.2f, enable_at_eval=True):\\n\" % p, model_do)\n"
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "source": [
    ...
    "model_do = dropout(model, p=0.2, enable_at_eval=True)\n",
    "\n",
    ...
    "    \"\"\"\n",
    "    Return predictive mean and variance under MC dropout.\n",
    "    Ensures we collect outputs of shape [T, N, out_dim].\n",
    "    \"\"\"\n",
    "    model_with_dropout.train()  # activate dropout layers\n",
    "    preds = []\n",
    "    for _ in range(T):\n",
    "        out = model_with_dropout(inputs)\n",
    "        preds.append(out.detach())\n",
    "    stacked = torch.stack(preds, dim=0)  # [T, N, out_dim]\n",
    "    mean = stacked.mean(dim=0)            # [N, out_dim]\n",
  ...
  },
  {
   ...
   "source": [
    "## 7. Part A Summary\n",
    "\n",
    "In Part A, we explored how Dropout works in deep learning and how the Dropout Transformation extends that idea in `probly`. Normal Dropout is a regularization technique used during training to reduce overfitting; the Dropout transformation repurposes the mechanism so that Dropout can remain active at inference to provide a measure of predictive uncertainty.\n",
    "\n",
    "We emphasised that the key difference is how we *use* randomness: normal dropout aims to improve training, while the transformation intentionally keeps randomness at prediction time to produce a predictive distribution that can be used to estimate uncertainty."
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "## Changelog — Seurays changes\n",
    "\"\"\"\n",
    "Changes included in this notebook (credited to Seurays):\n",
    "1. Ensure dropout(..., enable_at_eval=True) is used consistently where MC-Dropout is intended.\n",
    "2. Fix MC-sampling shape: collect outputs as tensors of shape [num_passes, C] rather than [num_passes, 1, C].\n",
    "3. Correct plotting and class-count logic (C = number of classes).\n",
    "4. Small text cleanups and completed truncated summary sections.\n",
    "\"\"\"\n"
   ]
  },
  {
   ...
   "source": [
    ...
    "\n",
    "We now transform the base model with `dropout()` so that Dropout stays *active* during inference (MC-Dropout)."
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    ...
    "mc_model.eval()  # we keep dropout layers configured to be active for MC-Dropout"
   ]
  },
  {
   "cell_type": "markdown",
   ...
   "source": [
    ...
    "\n",
    "We feed the same input through the model multiple times and collect the stochastic outputs. Note: for a single input sample we must remove the extra leading batch dimension when stacking results.\n"
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    ...
    "        out = mc_model(x)           # out shape: [1, C]\n",
    "        logits_list.append(out.squeeze(0))  # append [C] -> stack to [num_passes, C]\n",
    "\n",
    "logits = torch.stack(logits_list, dim=0)   # [num_passes, C]\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "std_probs  = probs.std(dim=0, unbiased=False)  # [C]\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "plt.hist(probs[:, winning_class].cpu().numpy(), bins=20, color=\"skyblue\", edgecolor=\"black\")\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "plt.figure(figsize=(6, 4))\n",
    "plt.bar(np.arange(C), mean_probs.cpu().numpy(), yerr=std_probs.cpu().numpy(), capsize=5, color=\"lightcoral\")\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "This shows the difference between a transformed model that keeps dropout active vs one that does not.\n"
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "source": [
    ...
    "We then computed the **mean**, **standard deviation**, and **predictive entropy** of the outputs, and visualized them using histograms and error-bar plots to understand how confident the model was about its predictions.\n",
    "\n",
    "Key takeaways:\n",
    "- Use enable_at_eval=True when you want MC-Dropout at inference.\n",
    "- Ensure you collect and stack outputs with correct shapes before computing statistics.\n",
    "- Interpreting uncertainty requires domain knowledge — absolute numerical values are not always meaningful without context.\n"
   ]
  },
  {
   "cell_type": "markdown",
   ...
   "source": [
    ...
    "Through this process, we transformed dropout from a tool that improves generalization into one that also provides valuable insight into a model’s **confidence and reliability**, bridging the gap between point predictions and predictive distributions.\n",
    "\n",
    "---\n"
   ]
  }
