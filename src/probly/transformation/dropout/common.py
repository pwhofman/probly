...
def register(cls: LazyType, traverser: RegisteredLooseTraverser) -> None:
    ...
    dropout_traverser.register(cls=cls, traverser=traverser, skip_if=is_first_layer, vars={"p": P, "rngs": RNGS})


def dropout[T: Predictor](base: T, p: float = 0.25, rngs: rnglib.Rngs | rnglib.RngStream | int = 1) -> T:
    ...
    return traverse(base, nn_compose(dropout_traverser), init={P: p, CLONE: True, RNGS: rngs})
