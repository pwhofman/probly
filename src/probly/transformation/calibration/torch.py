...
class TorchDirichletCalibrationPredictor[**In](_TorchCalibrationPredictorBase[In]):
    ...
    def __init__(self, predictor: nn.Module, config: CalibrationMethodConfig) -> None:
        ...
        self.register_buffer("_dirichlet_weight", torch.full((self.num_classes, self.num_classes), float("nan")))
        self.register_buffer("_dirichlet_bias", torch.full((self.num_classes,), float("nan")))
        self.register_buffer("_is_calibrated", torch.tensor(False, dtype=torch.bool))
...
def torch_calibration_generator(base: nn.Module, config: CalibrationMethodConfig) -> _TorchCalibrationPredictorBase:
    ...
    if config.method == "dirichlet":
        return TorchDirichletCalibrationPredictor(base, config)
