"""Shared RBF centroid-head transformation implementation."""
...
@runtime_checkable
class RBFCentroidHeadPredictor[**In, Out: DUQRepresentation](RepresentationPredictor[In, Out], Protocol):
...
@flexdispatch
def rbf_centroid_head_generator[**In, Out: DUQRepresentation](
    base: Predictor[In, Out],
    centroid_size: int,
    length_scale: float,
    gamma: float,
) -> RBFCentroidHeadPredictor[In, Out]:
    """Generate an RBF centroid-head model from a base model."""
    msg = f"No RBF centroid-head generator is registered for type {type(base)}"
    raise NotImplementedError(msg)


@predictor_transformation(permitted_predictor_types=(LogitClassifier,), preserve_predictor_type=False)
@RBFCentroidHeadPredictor.register_factory
def rbf_centroid_head[**In, Out: DUQRepresentation](
    base: Predictor[In, Out],
    ...
    Returns:
        The transformed RBF centroid-head predictor.
    """
    return rbf_centroid_head_generator(base, centroid_size, length_scale, gamma)
