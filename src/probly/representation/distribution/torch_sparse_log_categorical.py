...
class TorchSparseLogCategoricalDistribution(
    ...
    Shape: ``batch_shape``. The final axis of ``group_ids`` and ``entry_logits`` stores
    ...
    protected_axes: ClassVar[dict[str, int]] = {"group_ids": 1, "entry_logits": 1}
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.group_ids, torch.Tensor):
            ...
            raise TypeError(msg)
        if not isinstance(self.entry_logits, torch.Tensor):
            msg = "entry_logits must be a torch tensor."
            raise TypeError(msg)
        if self.group_ids.dtype not in (torch.int8, torch.int16, torch.int32, torch.int64, torch.uint8):
            ...
            raise TypeError(msg)
        if not torch.is_floating_point(self.entry_logits):
            msg = "entry_logits must be a floating point tensor."
            raise TypeError(msg)
        if self.group_ids.shape != self.entry_logits.shape:
            msg = "group_ids and entry_logits must have identical shapes."
            raise ValueError(msg)
        if self.group_ids.ndim < 1:
            msg = "group_ids and entry_logits must have at least one dimension."
            raise ValueError(msg)
        ...
        if torch.any(self.group_ids < 0):
            ...
            raise ValueError(msg)
        if torch.any(torch.isnan(self.entry_logits)) or torch.any(self.entry_logits == torch.inf):
            msg = "entry_logits must not contain NaN or positive infinity."
            raise ValueError(msg)
    ...
    def uniform_logits(self) -> TorchSparseLogCategoricalDistribution:
        ...
        """
        return replace(self, entry_logits=torch.zeros_like(self.entry_logits))

    def to_dense(self, num_classes: int | None = None) -> TorchProbabilityCategoricalDistribution:
        ...
        flat_logits = self.entry_logits.reshape((-1, sparse_size))
        ...
        return TorchProbabilityCategoricalDistribution(dense)
