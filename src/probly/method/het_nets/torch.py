...
from ._common import register, LAST_LAYER, NUM_FACTORS, TEMPERATURE, NUM_MC_SAMPLES, IS_PARAMETER_EFFICIENT, MULTILABEL
...
def drop_in_place_het_layer(obj: nn.Linear, state: State) -> tuple[nn.Module, State]:
    ...
    if state[LAST_LAYER]:
        ...
        return HeteroscedasticLayer(in_features=in_features, num_classes=num_classes, num_factors=state[NUM_FACTORS], temperature=state[TEMPERATURE], num_mc_samples=state[NUM_MC_SAMPLES], is_parameter_efficient=state[IS_PARAMETER_EFFICIENT], multilabel=state[MULTILABEL]), state
    return obj, state
