...
class DAREWrapper(nn.Module):
    """Wrapper for dare method."""

    def __init__(self, obj: nn.Module, delta: float, num_members: int, in_attr: int, out_attr: int) -> None:
        """Init."""
        super().__init__()
        self.obj = obj
        ...
        # s_raw (input-scaling) #noqa: ERA001
        self.s_raw = nn.Parameter(torch.ones(num_members, in_attr) + torch.randn(num_members, in_attr) * delta)
        # r_raw (output-scaling) #noqa: ERA001
        self.r_raw = nn.Parameter(torch.ones(num_members, out_attr) + torch.randn(num_members, out_attr) * delta)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward for dare wrapper."""
        m = self.num_members
        in_attr_expected = self.s_raw.shape[1]

        # If input doesn't match expected, something went wrong in model construction
        if x.shape[1] != in_attr_expected:
            msg = (
                f"DAREWrapper input mismatch: got {x.shape[1]} features, "
                f"expected {in_attr_expected}. This likely means the wrapped layer "
                f"({type(self.obj).__name__}) has wrong configuration."
            )
            raise ValueError(msg)

        original_batch_size = x.shape[0]
        # expand to ensemble_size if necessary
        if original_batch_size % m != 0:
            b = original_batch_size
            expand_shape = [m, *list(x.shape)]
            x = x.unsqueeze(0).expand(*expand_shape)
            x = x.reshape(m * b, *x.shape[2:])
        else:
            b = original_batch_size // m
        ...
        # input scaling (s)
        x_view = x.view(m, b, c, *spatial)
        s_actual = F.softplus(self.s_raw)  # ensures positivity
        s_view = s_actual.view(m, 1, c, *([1] * len(spatial)))
        x = (x_view * s_view).reshape(mb, c, *spatial)
        ...
        y_view = y.view(m, b, out_c, *spatial_out)
        r_actual = F.softplus(self.r_raw)  # ensures positivity
        r_view = r_actual.view(m, 1, out_c, *([1] * len(spatial_out)))
        y = (y_view * r_view).contiguous().view(mb, out_c, *spatial_out)

        return y

    def get_anti_reg_loss(self) -> torch.Tensor:
        """Returns a positive value, which describes the variance of the parameters."""
        s_var = self.s_raw.var(dim=0).mean()
        r_var = self.r_raw.var(dim=0).mean()
        return s_var + r_var
