"""=================================
Scaling Calibration Showcase - flax
=================================

Compare scaling-based calibration methods on a binary flax/nnx classifier.

We train a small nnx MLP with an explicit logit sharpening factor, which makes
it highly overconfident on noisy labels. We then calibrate the model outputs
with temperature, Platt, vector, and isotonic scaling and compare reliability
curves and probability histograms against the uncalibrated baseline.
"""
...
def _uniform_bin_stats(
    y_true: np.ndarray, probs: np.ndarray, n_bins: int
...
def _to_two_class_logits(binary_logits: jax.Array) -> jax.Array:
    logits_1d = binary_logits.reshape(-1)
    return jnp.stack([jnp.zeros_like(logits_1d), logits_1d], axis=-1)


class OverconfidentBinaryMLP(nnx.Module):
    """Small nnx MLP with fixed output sharpening for overconfidence."""

    def __init__(self, in_features: int, sharpening: float, *, rngs: nnx.Rngs) -> None:
        self.hidden1 = nnx.Linear(in_features, 64, rngs=rngs)
        self.hidden2 = nnx.Linear(64, 64, rngs=rngs)
        self.output = nnx.Linear(64, 1, rngs=rngs)
        self.sharpening = sharpening

    def __call__(self, x: jax.Array) -> jax.Array:
        h = nnx.relu(self.hidden1(x))
        h = nnx.relu(self.hidden2(h))
        logits = self.output(h).reshape(-1)
        return self.sharpening * logits


LogitClassifier.register(OverconfidentBinaryMLP)


def _train_loss(model: OverconfidentBinaryMLP, x: jax.Array, y: jax.Array) -> jax.Array:
    logits = model(x)
    return optax.sigmoid_binary_cross_entropy(logits, y).mean()


@nnx.jit
def _train_step(model: OverconfidentBinaryMLP, optimizer: nnx.Optimizer, x: jax.Array, y: jax.Array) -> None:
    grads = nnx.grad(_train_loss)(model, x, y)
...
X_train_j = jnp.asarray(X_train, dtype=jnp.float32)
X_calib_j = jnp.asarray(X_calib, dtype=jnp.float32)
X_test_j = jnp.asarray(X_test, dtype=jnp.float32)

y_train_j = jnp.asarray(y_train, dtype=jnp.float32)
y_calib_j = jnp.asarray(y_calib, dtype=jnp.float32)
y_test_j = jnp.asarray(y_test, dtype=jnp.float32)

# %%
# Train an intentionally overconfident MLP.
#
# A fixed logit sharpening factor makes the trained predictor overconfident.
# nnx has no `.backward()`; gradients come from `nnx.grad` and are applied
# through an `nnx.Optimizer` wrapping an optax transform.

model = OverconfidentBinaryMLP(in_features=X_train.shape[1], sharpening=LOGIT_SHARPENING, rngs=nnx.Rngs(RANDOM_SEED))
optimizer = nnx.Optimizer(model, optax.adam(1e-3), wrt=nnx.Param)

model.train()
for _ in range(TRAIN_EPOCHS):
    _train_step(model, optimizer, X_train_j, y_train_j)
model.eval()

calib_logits = model(X_calib_j)
test_logits = model(X_test_j)

uncalibrated_probs = np.asarray(nnx.sigmoid(test_logits)).reshape(-1)

# %%
# Calibrate with temperature, Platt, vector scaling, and isotonic regression.

temperature_model = temperature_scaling(flax_identity_logit_model())
platt_model = platt_scaling(flax_identity_logit_model())
vector_model = vector_scaling(flax_identity_logit_model(), num_classes=2)
isotonic_model = isotonic_regression(flax_identity_logit_model())

calibrate(temperature_model, y_calib_j, calib_logits)
calibrate(platt_model, y_calib_j, calib_logits)
calibrate(vector_model, y_calib_j.astype(jnp.int32), _to_two_class_logits(calib_logits))
calibrate(isotonic_model, y_calib_j, calib_logits)

temp_probs = np.asarray(nnx.sigmoid(predict_raw(temperature_model, test_logits))).reshape(-1)
platt_probs = np.asarray(nnx.sigmoid(predict_raw(platt_model, test_logits))).reshape(-1)
vector_logits_test = predict_raw(vector_model, _to_two_class_logits(test_logits))
vector_probs = np.asarray(jax.nn.softmax(vector_logits_test, axis=-1))[:, 1]
isotonic_probs = np.asarray(predict_raw(isotonic_model, test_logits)).reshape(-1)
...
y_test_np = np.asarray(y_test_j).astype(int)
...
fig.suptitle("Binary calibration: uncalibrated vs scaling methods (flax)", y=1.02)
plt.tight_layout()
plt.show()
