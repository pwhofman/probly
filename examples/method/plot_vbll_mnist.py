...
replace the final dense layer with a Bayesian last layer that maintains a
variational posterior over the output weights, emitting a closed-form Gaussian
over the logits whose variance grows on out-of-distribution inputs.
"""
...
# -----
#
# ``vbll`` replaces the classifier's linear head with a ``VBLLLayer`` holding a
# dense variational posterior over the last-layer weights.

base_model = MLPClassifier(in_features=28 * 28, hidden_features=256, out_features=10)
vbll_model = vbll(base_model, parameterization="dense")
...
# --------------------------
#
# The representer samples logits from the predictive Gaussian and softmaxes them
# into a second-order distribution over the classes.
...
plot = plot_mnist_uncertainty(
    ...
    uncertainty,
    mean_probs,
    title="Top-5 Most Uncertain Test Predictions (VBLL)",
)
plot.show()
