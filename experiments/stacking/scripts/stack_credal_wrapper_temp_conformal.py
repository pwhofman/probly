"""Experiment ``credal_wrapper_stack``: ensemble (credal_wrapper) -> calibration -> conformal RAPS.

Trains an N-member ensemble built via :func:`probly.method.credal_wrapper.credal_wrapper`
(same training-time object as ``ensemble``; only the inference-time
representer differs) on the chosen dataset, pools member logits by
mean, and runs the conformal layer on top. The calibration step is
selected via ``--calibration``:
    ...
    label_relaxation  LabelRelaxationLoss at training time, no post-hoc
...
ECE_BINS = 15
EXPERIMENT = "credal_wrapper_stack"
...
def main() -> None:
    """Run credal_wrapper_stack on the chosen dataset / encoder / calibration mode."""
    args = _parse_args()
    set_seed(args.seed)
